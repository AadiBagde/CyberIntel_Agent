...
from backend.workflows.nodes.deduplicate import create_deduplicate_node
...
def create_phase3_registry(
    ...
    registry.register("deduplicate_node", create_deduplicate_node(deduplication_agent, repo))
...
def compile_phase3_graph(
    session: AsyncSession,
    research_agent: ResearchAgent,
    deduplication_agent: DeduplicationAgent,
    analysis_agent: ThreatAnalysisAgent,
) -> CompiledStateGraph:
    registry = create_phase3_registry(
        session,
        research_agent,
        deduplication_agent,
        analysis_agent,
    )
    return build_graph_with_order(registry, PHASE3_NODE_ORDER).compile()
